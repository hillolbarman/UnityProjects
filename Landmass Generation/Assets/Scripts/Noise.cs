	...
	public static float[,] GenerateNoiseMap(int mapWidth, int mapHeight, int seed, float scale, int octaves, float persistance, float lacunarity, Vector2 offset, NormalizedMode normalizedMode)
	{
		...
		for (int y = 0; y < mapHeight; y++)
		{
			for (int x = 0; x < mapWidth; x++)
			{
                ...
                {
					//float normalisedValue = (noiseMap[x, y] - 1) / (2f * maxPossibleHeight / 1.75f);
					float normalisedValue = (noiseMap[x, y] -1 + maxPossibleHeight) / (maxPossibleHeight );
					noiseMap[x, y] = Mathf.Clamp(normalisedValue,0,int.MaxValue);
                }
			}
		}
